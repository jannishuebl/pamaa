(function($){
    $.fn.Dropdown = function( options ){
        ...
        var initSelectors = function(selectors){
        	console.debug(selectors.children('a'));
        	selectors.children('a').off("click.dropdown");
            selectors.children('a').on('click.dropdown', function(e){
                //e.stopPropagation();
                //$("[data-role=dropdown]").removeClass("active");
                
            	if($(e.originalEvent.target).parent('li').is("[data-role]")) e.stopPropagation();
				
                clearDropdown();
                $(this).parents("ul").css("overflow", "visible");
console.debug(this);
                var $m = $(this).parent('li').children(".dropdown-menu, .sidebar-dropdown-menu");
